...
class DDPApexProcessor(object):
    def __init__(self, cfg_path):
        ...
        self.creterion = CenterNetLoss(cls_weight=self.hyper_params['cls_weight'],
                                       wh_weight=self.hyper_params['wh_weight'],
                                       reg_weight=self.hyper_params['reg_weight'])
    ...
    def train(self, epoch):
        ...
        for i, (img_tensor, targets_tensor, _) in enumerate(pbar):
            ...
            with torch.no_grad():
                ...
                gt_dict = CenterNetGT.generate(targets_tensor,img_tensor.shape[0],self.model_cfg['num_cls'],
                                               down_scale=self.model_cfg['DOWN_SCALE'],
                                               output_size=self.model_cfg['output_size'],
                                               min_overlap=self.model_cfg['MIN_OVERLAP'],
                                               tensor_dim=self.model_cfg['TENSOR_DIM'])
                for k in gt_dict:
            ...
            total_loss, detail_loss = self.creterion(pred_dict, gt_dict)
            total_loss.backward()
            ...
            loss_cls, loss_wh, loss_reg = detail_loss
            loss_list[0].append(total_loss.item())
            loss_list[1].append(loss_cls.item())
            loss_list[2].append(loss_wh.item())
            loss_list[3].append(loss_reg.item())
            if self.local_rank == 0:
                pbar.set_description(
                    "epoch:{:2d}|size:{:3d}|target_loss:{:6.4f}|loss_cls:{:6.4f}|loss_wh:{:6.4f}|loss_reg:{:6.4f}|lr:{:8.6f}".format(
                        epoch + 1,
                        h,
                        total_loss.item(),
                        loss_cls.item(),
        ...
        print(
            "epoch:{:3d}|local:{:3d}|target_loss:{:6.4f}|loss_cls:{:6.4f}|loss_wh:{:6.4f}|loss_reg:{:6.4f}|lr:{:8.6f}"
                .format(epoch + 1,
                        self.local_rank,
                        mean_loss_list[0],
                        mean_loss_list[1],
    ...
    def val(self, epoch):
        ...
        for img_tensor, targets_tensor, _ in pbar:
            ...
            predicts[0][:, [0, 2]] = predicts[0][:, [0, 2]].clamp(min=0, max=w)
            predicts[0][:, [1, 3]] = predicts[0][:, [1, 3]].clamp(min=0, max=h)
