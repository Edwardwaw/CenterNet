...
from commons.augmentations2 import *
...
default_aug_cfg = {
    'boarder': 128,
    'random_aug': True,
    'brightness_intensity_min': 0.6,
    'brightness_intensity_max': 1.4,
    'contrast_intensity_min': 0.6,
    'contrast_intensity_max': 1.4,
    'saturation_intensity_min': 0.6,
    'saturation_intensity_max': 1.4,
    'lighting_scale': 0.1,
    'pad_val': (123,116,103),
}
...
class COCODataSets(Dataset):
    ...
    def __getitem__(self, item):
        ...
        img = np.asarray(img)
        '''
        img(np.array):  np.int(0-255)   format=RGB   shape=HWC
        '''
    ...
    def set_transform(self):
        if self.augments:
            basic_transform = Compose(transforms=[
                CenterAffine(boarder=self.aug_cfg['boarder'],output_size=(self.img_size,self.img_size),random_aug=self.aug_cfg['random_aug']),
                RandomFlip(),
                RandomBrightness(intensity_min=self.aug_cfg['brightness_intensity_min'],intensity_max=self.aug_cfg['brightness_intensity_max']),
                RandomContrast(intensity_min=self.aug_cfg['contrast_intensity_min'],intensity_max=self.aug_cfg['contrast_intensity_max']),
                RandomSaturation(intensity_min=self.aug_cfg['saturation_intensity_min'],intensity_max=self.aug_cfg['saturation_intensity_max']),
                RandomLighting(scale=self.aug_cfg['lighting_scale'])
            ])

            self.transform = basic_transform
        else:
            self.transform = ScalePadding(target_size=(self.img_size,self.img_size),padding_val=self.aug_cfg['pad_val'])
