...
from commons.augmentations2 import CenterAffine
from utils.centernet import gather_feature
...
class CenterNetDecoder(object):
    ...
    def decode(fmap, wh, reg=None, cat_spec_wh=False, K=100):
        ...
        scores, index, clses, ys, xs = CenterNetDecoder.topk_score(fmap, K=K)  # shape=[batch,K]
        if reg is not None:
        ...
        '''
        bboxes: [batch,K,4] 4==>x1,y1,x2,y2
        scores: [batch,K,1]
        clses:  [batch,K,1]
        '''
    ...
    def transform_boxes(boxes):
        ...
        """
        boxes = boxes.cpu().numpy().reshape(-1, 4)*4
        return boxes
    ...
    def topk_score(scores, K=40):
        ...
        # get topk score and its index in every H x W(channel dim) feature map, that is by classification
        # shape=[batch,channel,K]
        ...
        # get all topk in a batch
        # shape=[batch,K]
        topk_score, index = torch.topk(topk_scores.reshape(batch, -1), K)
        # div by K because index is grouped by K(C x K shape)
        topk_clses = (1.0*index / K).int()
